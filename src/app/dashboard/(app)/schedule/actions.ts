...
async function requireScheduleAccess(companyId?: string) {
  ...
  if (!identity) return { ok: false as const, error: "Unauthorized" }
  ...
  if (!(await requireOwnerAccess(identity.userId, identity.userEmail, company))) return { ok: false as const, error: "Not available for your account" }
