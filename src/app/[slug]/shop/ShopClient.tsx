          ...
          <section className="box-border max-h-[88svh] w-full max-w-[100svw] overscroll-contain overflow-x-hidden overflow-y-auto rounded-t-[34px] bg-white shadow-2xl md:max-w-3xl md:rounded-[34px]" onClick={(event) => event.stopPropagation()}>
            <div className="grid md:grid-cols-[1.05fr_0.95fr]">
              <div className="bg-neutral-100 p-4">
                <div className="aspect-square overflow-hidden rounded-[26px] bg-neutral-50 p-5">
                  {selectedImage ? <img src={selectedImage} alt={selectedProduct.name} className="h-full w-full object-contain" /> : <div className="relative flex h-full items-center justify-center overflow-hidden" style={{ background: `linear-gradient(135deg, ${primary}18, #F5F7F6)` }}><div className="absolute inset-0 opacity-70" style={{ background: "radial-gradient(circle at 28% 24%, rgba(255,255,255,0.95), transparent 32%), radial-gradient(circle at 78% 76%, rgba(0,0,0,0.08), transparent 34%)" }} /><span className="relative text-4xl font-black" style={{ color: primary }}>{itemInitials(selectedProduct.name)}</span></div>}
                </div>
                {productImages(selectedProduct).length > 1 && <div className="mt-3 grid grid-cols-5 gap-2">{productImages(selectedProduct).map((image) => <button key={image} onClick={() => setSelectedImage(image)} className="aspect-square overflow-hidden rounded-xl border bg-neutral-50 p-1" style={{ borderColor: selectedImage === image ? primary : "#e5e5e5" }}><img src={image} alt="" className="h-full w-full object-contain" /></button>)}</div>}
              </div>
