...
function pickupReceiptText(businessName: string, pickupTime: string, pickupLocation?: { name?: string | null; address?: string | null } | null) {
  ...
  if (address) lines.push(`Pickup at: ${[name || businessName, address].filter(Boolean).join("\n")}`)
  else lines.push(`${businessName} will contact you with pickup instructions.`)
  return lines.join("\n")
}
...
export async function POST(req: NextRequest) {
  ...
  const limit = checkPublicRateLimit(req, { key: `online-order-complete:${companyId}:${leadId}`, limit: 20, windowMs: 5 * 60 * 1000 })
