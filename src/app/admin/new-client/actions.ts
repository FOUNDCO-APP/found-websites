      ...
      <table width="100%" style="max-width:520px;background:#ffffff;border-radius:16px;overflow:hidden;">
        <tr>
          <td style="background:#080A09;padding:32px;text-align:center;">
            <p style="margin:0 0 6px;font-size:11px;font-weight:700;letter-spacing:3px;text-transform:uppercase;color:#32D074;">Your website is live</p>
...
export async function deferClientBilling(formData: FormData) {
  ...
  const activityLines = [
    `Deferred billing set: card due by ${dueDateLabel} (${termDays} days). If no card is added by then, the public site pauses automatically. Reason: ${reason}`,
  ]
    ...
    if (company?.email) {
      try {
        ...
        await resend.emails.send({
          ...
          html: buildAddCardEmail({
            businessName: company.name,
            siteUrl: `https://${company.slug}.${ROOT_DOMAIN}`,
            activateUrl: `https://${ROOT_DOMAIN}/activate?slug=${company.slug}`,
            dueDateLabel,
          }),
          text: `Your website is live: https://${company.slug}.${ROOT_DOMAIN}\n\nAdd a card to keep it running - nothing is charged today, billing starts ${dueDateLabel}.\n\nAdd your card: https://${ROOT_DOMAIN}/activate?slug=${company.slug}\n\n- The Found team`,
        })
        activityLines.push(`Card-link email sent to ${company.email}.`)
      } catch (err) {
        console.error("[admin/new-client] add-card email failed:", err)
        activityLines.push(`Card-link email FAILED to send to ${company.email} - send the link manually.`)
      }
    }
  }
