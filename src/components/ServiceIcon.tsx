const paths: Record<string, string> = {
  ...
  plumbing:     "M11.42 15.17L17.25 21A2.652 2.652 0 0021 17.25l-5.877-5.877M11.42 15.17l2.496-3.03c.317-.384.74-.626 1.208-.766M11.42 15.17l-4.655 5.653a2.548 2.548 0 11-3.586-3.586l6.837-5.63m5.108-.233c.55-.164 1.163-.188 1.743-.14a4.5 4.5 0 004.486-6.336l-3.276 3.277a3.004 3.004 0 01-2.25-2.25l3.276-3.276a4.5 4.5 0 00-6.336 4.486c.091 1.076-.071 2.264-.904 2.95l-.102.085m-1.745 1.437L5.909 7.5H4.5L2.25 3.75l1.5-1.5L7.5 4.5v1.409l4.26 4.26m-1.745 1.437l1.745-1.437m6.615 8.206L15.75 15.75M4.867 19.125h.008v.008h-.008v-.008z",
  remodeling:   "M21.75 6.75a4.5 4.5 0 01-4.884 4.484c-1.076-.091-2.264.071-2.95.904l-7.152 8.684a2.548 2.548 0 11-3.586-3.586l8.684-7.152c.833-.686.995-1.874.904-2.95a4.5 4.5 0 016.336-4.486l-3.276 3.276a3.004 3.004 0 002.25 2.25l3.276-3.276c.256.565.398 1.192.398 1.852z",
  ...
  windows:      "M3.75 6A2.25 2.25 0 016 3.75h2.25A2.25 2.25 0 0110.5 6v2.25a2.25 2.25 0 01-2.25 2.25H6a2.25 2.25 0 01-2.25-2.25V6zM3.75 15.75A2.25 2.25 0 016 13.5h2.25a2.25 2.25 0 012.25 2.25V18a2.25 2.25 0 01-2.25 2.25H6A2.25 2.25 0 013.75 18v-2.25zM13.5 6a2.25 2.25 0 012.25-2.25H18A2.25 2.25 0 0120.25 6v2.25A2.25 2.25 0 0118 10.5h-2.25a2.25 2.25 0 01-2.25-2.25V6zM13.5 15.75a2.25 2.25 0 012.25-2.25H18a2.25 2.25 0 012.25 2.25V18A2.25 2.25 0 0118 20.25h-2.25A2.25 2.25 0 0113.5 18v-2.25z",
  flooring:     "M6.429 9.75L2.25 12l4.179 2.25m0-4.5l5.571 3 5.571-3m-11.142 0L2.25 7.5 12 2.25l9.75 5.25-4.179 2.25m0 0L21.75 12l-4.179 2.25m0 0l4.179 2.25L12 21.75 2.25 16.5l4.179-2.25m11.142 0l-5.571 3-5.571-3",
  insulation:   "M12 18v-5.25m0 0a6.01 6.01 0 001.5-.189m-1.5.189a6.01 6.01 0 01-1.5-.189m3.75 7.478a12.06 12.06 0 01-4.5 0m3.75 2.383a14.406 14.406 0 01-3 0M14.25 18v-.192c0-.983.658-1.823 1.508-2.316a7.5 7.5 0 10-7.517 0c.85.493 1.509 1.333 1.509 2.316V18",
  landscaping:  "M12 3v2.25m6.364.386l-1.591 1.591M21 12h-2.25m-.386 6.364l-1.591-1.591M12 18.75V21m-4.773-4.227l-1.591 1.591M5.25 12H3m4.227-4.773L5.636 5.636M15.75 12a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0z",
  cleaning:     "M9.813 15.904L9 18.75l-.813-2.846a4.5 4.5 0 00-3.09-3.09L2.25 12l2.846-.813a4.5 4.5 0 003.09-3.09L9 5.25l.813 2.846a4.5 4.5 0 003.09 3.09L15.75 12l-2.846.813a4.5 4.5 0 00-3.09 3.09zM18.259 8.715L18 9.75l-.259-1.035a3.375 3.375 0 00-2.455-2.456L14.25 6l1.036-.259a3.375 3.375 0 002.455-2.456L18 2.25l.259 1.035a3.375 3.375 0 002.456 2.456L21.75 6l-1.035.259a3.375 3.375 0 00-2.456 2.456z",
  // Industry fallbacks
...
export function getServiceIcon(serviceName: string): string {
  ...
  if (n.includes("paint") || n.includes("stain") || n.includes("coat")) return "painting"
  if (n.includes("electric") || n.includes("wiring") || n.includes("panel") || n.includes("outlet")) return "electrical"
  if (n.includes("plumb") || n.includes("pipe") || n.includes("drain") || n.includes("water heater")) return "plumbing"
  if (n.includes("remodel") || n.includes("addition") || n.includes("convert")) return "remodeling"
  if (n.includes("renovat") || n.includes("restore") || n.includes("repair")) return "construction"
  if (n.includes("hvac") || n.includes("heat") || n.includes("cool") || n.includes("air condition") || n.includes("ventil") || n.includes("furnace")) return "hvac"
  if (n.includes("construct") || n.includes("build") || n.includes("frame") || n.includes("adu") || n.includes("new home")) return "construction"
  if (n.includes("window") || n.includes("door") || n.includes("glass") || n.includes("sliding")) return "windows"
  if (n.includes("floor") || n.includes("carpet") || n.includes("tile") || n.includes("hardwood") || n.includes("laminate")) return "flooring"
  ...
  if (n.includes("food") || n.includes("catering") || n.includes("meal") || n.includes("chef") || n.includes("menu")) return "food"
  if (n.includes("shop") || n.includes("retail") || n.includes("product") || n.includes("merch")) return "retail"
  if (n.includes("fitness") || n.includes("train") || n.includes("workout") || n.includes("gym") || n.includes("class")) return "fitness"
  return "default"
}
...
export default function ServiceIcon({
  ...
  const key = getServiceIcon(serviceName)
  const path = paths[key] ?? paths.default
