...
type Vocab = {
  slowHint: string; slowBody: string
  followHint: string; followBody: string
  seasonalBody: string; reengageBody: string
}
  ...
  switch (industry) {
    ...
    case "pet_services":
      return {
        ...
        slowBody: `Hey {firstName},\n\nWe have some openings this week and wanted to reach out to see if your pet is due for a visit.\n\nIf you've been meaning to come in, now's a great time — we'd love to see you both.\n\nReply to this email or give us a call to book an appointment.\n\n— `,
        followHint: "Check in after a pet's appointment.",
        followBody: `Hey {firstName},\n\nJust wanted to follow up after your pet's last appointment. We hope everything went smoothly!\n\nIf you have any questions or there's anything we can help with, don't hesitate to reach out. A quick review would mean a lot to us.\n\n— `,
        ...
        reengageBody: `Hey {firstName},\n\nIt's been a little while since we've seen you and your pet, and we wanted to check in.\n\nIf it's time for an appointment or you have any questions about their care, we're always here. We'd love to see you both again soon.\n\n— `,
      }
    case "wellness":
    case "beauty":
    case "fitness":
      return {
        ...
        reengageBody: `Hey {firstName},\n\nWe noticed it's been a little while and we wanted to reach out.\n\nIf there's anything that kept you away or something we can do better, just reply — we'd genuinely love to hear it. Whenever you're ready, we're here for you.\n\n— `,
      }
    default:
      return {
        slowHint: "Fill a slow week with a time-limited offer.",
        ...
        followBody: `Hey {firstName},\n\nJust wanted to follow up and see how everything went. We hope you were happy with the work!\n\nIf you have any questions or there's anything we can help with, don't hesitate to reach out. A quick review would mean the world to us.\n\n— `,
        ...
        reengageBody: `Hey {firstName},\n\nIt's been a little while since we've connected and we wanted to reach out.\n\nIf there's ever anything we can help with or if you have questions, just reply — we're here. Hope to hear from you soon.\n\n— `,
      }
  }
}
...
function getTemplates(vocab: Vocab): Template[] {
  return [
...
function TemplateIcon({ slug, color }: { slug: string; color: string }) {
  ...
  switch (slug) {
    case "slow_week":      return <svg width="16" height="16" viewBox="0 0 24 24" {...s}><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg>
    case "new_service":    return <svg width="16" height="16" viewBox="0 0 24 24" {...s}><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="16"/><line x1="8" y1="12" x2="16" y2="12"/></svg>
    case "seasonal":       return <svg width="16" height="16" viewBox="0 0 24 24" {...s}><circle cx="12" cy="12" r="5"/><line x1="12" y1="1" x2="12" y2="3"/><line x1="12" y1="21" x2="12" y2="23"/><line x1="4.22" y1="4.22" x2="5.64" y2="5.64"/><line x1="18.36" y1="18.36" x2="19.78" y2="19.78"/><line x1="1" y1="12" x2="3" y2="12"/><line x1="21" y1="12" x2="23" y2="12"/><line x1="4.22" y1="19.78" x2="5.64" y2="18.36"/><line x1="18.36" y1="5.64" x2="19.78" y2="4.22"/></svg>
    case "holiday":        return <svg width="16" height="16" viewBox="0 0 24 24" {...s}><path d="M20 12v10H4V12"/><path d="M22 7H2v5h20V7z"/><path d="M12 22V7"/><path d="M12 7H7.5a2.5 2.5 0 0 1 0-5C11 2 12 7 12 7z"/><path d="M12 7h4.5a2.5 2.5 0 0 0 0-5C13 2 12 7 12 7z"/></svg>
    case "thank_you":      return <svg width="16" height="16" viewBox="0 0 24 24" {...s}><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg>
    case "re_engagement":  return <svg width="16" height="16" viewBox="0 0 24 24" {...s}><polyline points="1 4 1 10 7 10"/><path d="M3.51 15a9 9 0 1 0 .49-3.5"/></svg>
    case "announcement":   return <svg width="16" height="16" viewBox="0 0 24 24" {...s}><path d="M18 8a2 2 0 0 1 0 4"/><path d="M10 8v11a1 1 0 0 1-1 1H7a1 1 0 0 1-1-1v-5"/><path d="M12 8H4l-2 4 2 4h8l6-4-6-4z"/></svg>
    case "birthday":       return <svg width="16" height="16" viewBox="0 0 24 24" {...s}><path d="M20 21v-8a2 2 0 0 0-2-2H6a2 2 0 0 0-2 2v8"/><path d="M4 16s.5-1 2-1 2.5 2 4 2 2.5-2 4-2 2.5 2 4 2 2-1 2-1"/><line x1="2" y1="21" x2="22" y2="21"/><path d="M7 8v1"/><path d="M12 8v1"/><path d="M17 8v1"/></svg>
    case "follow_up":      return <svg width="16" height="16" viewBox="0 0 24 24" {...s}><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>
    default:               return <svg width="16" height="16" viewBox="0 0 24 24" {...s}><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>
  }
}
...
function Sheet({ open, onClose, title, children }: {
  ...
  return createPortal(
    <div onClick={onClose} style={{ position: "fixed", inset: 0, zIndex: 9998, backgroundColor: "rgba(0,0,0,0.6)", display: "flex", alignItems: "flex-end" }}>
      <div onClick={e => e.stopPropagation()} style={{ width: "100%", maxHeight: "82vh", overflowY: "auto", backgroundColor: "#1C1C1E", borderRadius: "20px 20px 0 0", paddingBottom: "env(safe-area-inset-bottom, 20px)" }}>
...
async function shareOrDownloadQR(subscribeUrl: string, companySlug: string) {
...
async function shareOrCopyLink(url: string): Promise<boolean> {
  if (typeof navigator !== "undefined" && navigator.share) {
    try {
      await navigator.share({ url, title: "Join our subscriber list" })
      return true
...
export default function MarketingClient({
  companyId, companyName, companySlug, industry,
  contacts, campaigns, rootDomain,
}: {
  companyId: string; companyName: string; companySlug: string; industry: string | null
  contacts: Contact[]; campaigns: Campaign[]; rootDomain: string
}) {
  const [step, setStep] = useState<"home" | "compose" | "sent">("home")
  ...
  async function handleSend() {
    ...
    try {
      ...
      if (!res.ok) { setError(data.error ?? "Send failed."); setShowConfirm(false); return }
  ...
  async function handleLinkShare() {
    const didShare = await shareOrCopyLink(subscribeUrl)
    if (!didShare) {
      setLinkState("copied")
  ...
  async function handleQR() {
    setQrWorking(true)
    try { await shareOrDownloadQR(subscribeUrl, companySlug) } finally { setQrWorking(false) }
  }

  // Shared styles
  const card: React.CSSProperties = { borderRadius: 16, backgroundColor: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.07)" }
  const inputStyle: React.CSSProperties = { width: "100%", boxSizing: "border-box", backgroundColor: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 12, padding: "13px 14px", ...TYPE.footnote, color: "white", outline: "none", resize: "none" as const }
  const qrImgUrl = `https://api.qrserver.com/v1/create-qr-code/?size=320x320&data=${encodeURIComponent(subscribeUrl)}&margin=16`

  // Audience filter sheet (shared between compose + confirm sheet)
  const audienceSheet = (
    <Sheet open={showAudience} onClose={() => setShowAudience(false)} title="Who are you sending to?">
      <div style={{ padding: "0 20px 24px" }}>
        {(Object.entries(segments) as [FilterKey, typeof segments[FilterKey]][]).map(([key, seg]) => (
          <button key={key} onClick={() => { setFilter(key); setShowAudience(false) }}
  ...
  if (step === "sent") {
    return (
      <main style={{ padding: "40px 20px", textAlign: "center", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "60vh" }}>
        ...
        <p style={{ margin: "0 0 28px", ...TYPE.body, color: `rgba(255,255,255,${TEXT_OPACITY.tertiary})` }}>Your email is on its way.</p>
  ...
  if (step === "compose") {
    return (
      <main style={{ padding: "20px" }}>
        ...
        {/* TO: row */}
        <button onClick={() => setShowAudience(true)}
        ...
        <input style={{ ...inputStyle, marginBottom: 16 }} type="text" value={subject} onChange={e => setSubject(e.target.value)} placeholder="Email subject" />

        <p style={{ margin: "0 0 6px", ...TYPE.caption, color: `rgba(255,255,255,${TEXT_OPACITY.tertiary})` }}>
          Message <span style={{ color: `rgba(255,255,255,0.22)` }}>— tap to edit · use {"{firstName}"} to personalize</span>
        </p>
        <textarea style={{ ...inputStyle, minHeight: 240 }} value={body} onChange={e => setBody(e.target.value)} placeholder="Your message…" />

        {error && <p style={{ margin: "8px 0 0", ...TYPE.caption, color: "#F43F5E", fontWeight: 700 }}>{error}</p>}

        <button onClick={() => { setError(null); setShowConfirm(true) }}
        ...
        {/* Confirm sheet */}
        <Sheet open={showConfirm} onClose={() => !sending && setShowConfirm(false)} title="Ready to send?">
          <div style={{ padding: "0 20px 28px" }}>
            <div style={{ padding: "14px 16px", borderRadius: 12, backgroundColor: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.07)", marginBottom: 14 }}>
              <p style={{ margin: "0 0 2px", ...TYPE.caption, color: `rgba(255,255,255,${TEXT_OPACITY.tertiary})` }}>Subject</p>
              <p style={{ margin: 0, ...TYPE.subhead, fontWeight: 600, color: "white" }}>{subject}</p>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "12px 16px", borderRadius: 12, backgroundColor: `${GREEN}0D`, border: `1px solid ${GREEN}20`, marginBottom: 20 }}>
              <span style={{ width: 7, height: 7, borderRadius: "50%", backgroundColor: GREEN, flexShrink: 0 }} />
            ...
            <button onClick={handleSend} disabled={sending || recipientCount === 0}
              ...
              {sending ? "Sending…" : recipientCount === 0 ? "No recipients" : `Send to ${recipientCount} ${recipientCount === 1 ? "person" : "people"} →`}
            </button>
          </div>
        </Sheet>
      </main>
  ...
  return (
    <main style={{ padding: "24px 20px 40px" }}>
      ...
      <div style={{ ...card, padding: "22px 22px 20px", marginBottom: 16 }}>
        ...
        <p style={{ margin: "0 0 18px", ...TYPE.footnote, color: `rgba(255,255,255,${TEXT_OPACITY.tertiary})` }}>
      ...
      <div style={{ ...card, padding: "22px 22px 20px", marginBottom: 16 }}>
        ...
        <div style={{ display: "flex", justifyContent: "center", marginBottom: 14 }}>
          <div style={{ backgroundColor: "white", borderRadius: 14, padding: 12 }}>
            ...
            <img src={qrImgUrl} alt="Subscribe QR code" width={160} height={160} style={{ display: "block" }} />
          </div>
        </div>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          <button onClick={handleQR} disabled={qrWorking}
            style={{ flex: 1, padding: "11px 0", borderRadius: 10, backgroundColor: "rgba(255,255,255,0.07)", border: "1px solid rgba(255,255,255,0.1)", color: "white", ...TYPE.footnote, fontWeight: 700, cursor: "pointer", opacity: qrWorking ? 0.5 : 1 }}>
            {qrWorking ? "Working…" : "Share QR"}
          </button>
          <button onClick={handleLinkShare}
            style={{ flex: 1, padding: "11px 0", borderRadius: 10, backgroundColor: linkState === "copied" ? `${GREEN}18` : "rgba(255,255,255,0.07)", border: `1px solid ${linkState === "copied" ? `${GREEN}35` : "rgba(255,255,255,0.1)"}`, color: linkState === "copied" ? GREEN : "white", ...TYPE.footnote, fontWeight: 700, cursor: "pointer", transition: "all 0.2s" }}>
            {linkState === "copied" ? "Copied!" : "Share Link"}
          </button>
        </div>
        <p style={{ margin: "10px 0 0", ...TYPE.caption, color: `rgba(255,255,255,0.15)`, textAlign: "center" as const, wordBreak: "break-all" as const }}>
          {subscribeUrl}
        </p>
      </div>
      ...
      {campaigns.length > 0 && (
        <div>
          <p style={{ margin: "0 0 8px", ...TYPE.caption, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase" as const, color: `rgba(255,255,255,${TEXT_OPACITY.disabled})` }}>Sent</p>
            ...
            {campaigns.slice(0, 5).map((c, i) => (
              <div key={c.id} style={{ padding: "13px 16px", borderBottom: i < Math.min(campaigns.length, 5) - 1 ? "1px solid rgba(255,255,255,0.05)" : "none", display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12 }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <p style={{ margin: "0 0 2px", ...TYPE.footnote, fontWeight: 600, color: "white", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" as const }}>{c.subject}</p>
