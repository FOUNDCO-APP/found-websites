...
- [ ] T-Shirts connected-account payment proof - complete one live shop-order payment on `tshirts`, or directly reconcile `acct_1TtMCKEuYeP9g9CO` in Stripe Dashboard, before claiming every connected profile has a successful payment test. During that test, choose Shipping and confirm native address autofill plus readable owner/customer shipping address output.
