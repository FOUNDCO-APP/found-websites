...
- **Domain registrar auto-setup (real DNS automation, not just instructions)** - research done 2026-07-30, no code yet. Target: GoDaddy first (dominant registrar among likely owners). Two real paths found: Domain Connect protocol (no stored credentials, but needs GoDaddy's external approval - unknown timeline) and GoDaddy's scoped Personal Access Token API (usable today, no external approval, but requires the owner to generate+hand over a token and Found to store it). Decision: pursue Domain Connect as the goal, scoped-PAT as the documented fallback, don't block v1 on Domain Connect's approval clock. **Needs explicit sign-off before any credential-handling code gets written** - this is real registrar-credential custody territory. See `SESSION_HANDOFF.md` and `DECISIONS.md` [2026-07-30] for full detail. Namecheap/other registrars deferred until the GoDaddy path is proven.
