...
- [x] Root-caused and fixed: the industry desktop grid was nested inside `lg:grid-cols-[0.82fr_1.18fr]`, so it only had ~59% of a 1024px container instead of full page width like Home/Plans.
...
- [ ] Shawn QA: tap a homepage pricing card on a real iPhone and confirm the CTA directly below updates without scrolling; check `/compare` and `/industries/*` on a real desktop/tablet width (not just phone) and confirm the 3-card row is full width with no squeezed/wrapped text; confirm the Recommended badge still isn't clipped anywhere.
