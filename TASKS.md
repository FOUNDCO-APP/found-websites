...
- [x] MBJ / Divine Remodel / RC Bicycles regenerated by Shawn (Aug 27 AM) - all three verified serving real BMP favicon.ico + correct apple-touch-icon.
- [x] Bare `/favicon.ico` + `/apple-touch-icon.png` now `no-store` (commit `7214112`) - iOS hits these unversioned paths by convention; they were edge-cacheable and freezing stale into iOS's store.
- [x] Admin bulk-regen endpoint shipped: `POST /api/admin/regenerate-site-icons?key=ADMIN_KEY`.
- [ ] Shawn's own Apple devices still show stale icons (iCloud-synced favicon cache). Servers are correct; real visitors see the right icon. Purge requires clearing Safari data on iPhone + iPad + Mac together, or wait ~days.
- [ ] Optional: run the bulk-regen endpoint for the ~27 remaining practice sites.
- [ ] Future (Jony): render 16-32px favicons from a simplified mark so detailed logos stay legible.

### Admin "View As" Regression (FIXED, commit `bf68ca6`)

- [x] Root cause: worker-role lockdown added `if (!user) redirect("/login")` gates that don't honor the admin-only "View As" session.
- [x] New `resolveDashboardIdentity()`; refactored entitlements, site/schedule/photos actions, leads layout, ~14 API routes.
- [x] `npx tsc --noEmit` + `npm run build` pass; pushed.
- [ ] Shawn QA after deploy: from admin "View As" a client (no separate login), confirm icon upload, estimates, schedule, leads, marketing, contacts all work.
