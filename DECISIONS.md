**[2026-08-25] - Job/project albums are private by default; website gallery requires explicit owner approval.**
Approved by: Shawn + Steve Jobs + Jony Ive + Angela Ahrendts + Craig Federighi + Priya Nair + Marcus Webb + Chris Lattner
Why: Jobs/projects are work records and shareable client galleries, not automatic public marketing albums. A customer job appearing on a business website without the owner's explicit choice is a trust and privacy failure. Decision: public `/gallery` shows only albums marked for website display. Job/project albums default private and stay accessible by direct shared link for the customer. Owners get a clear `Show on website gallery` toggle near the job share controls when they intentionally want to feature a project publicly.

**[2026-08-24] - Found owns the billing experience; Stripe only powers secure payment tasks.**
Approved by: Shawn + Steve Jobs + Jony Ive + Angela Ahrendts + Craig Federighi + Priya Nair + Phil Schiller
Why: Shawn reviewed a live customer's Plan & Billing page and rejected sending owners into a generic Stripe portal where the experience stops feeling like Found and cancellation can sit too casually beside card updates. Billing is a trust moment. Decision: the owner-facing Plan & Billing page must be the billing home: plan, price, status, card signal, next billing date, invoices, and cancellation/help context live inside Found. Stripe remains the source of truth and the secure processor for card entry, invoices, subscriptions, and compliance, but generic Stripe portal access is not the main product experience. Customer cancellation/plan-help should be a Found-controlled flow that explains what happens to the site, domain, leads, photos, and billing timing before anything turns off. Stripe-hosted pages may still open for focused secure tasks such as updating a card or viewing invoices, but portal configurations must disable subscription cancellation and plan changes unless a later team decision explicitly reopens that.
...
**[2026-08-16] - Domain automation resumes as Domain Connect feasibility proof, not direct registrar API.**
Approved by: Shawn (continuation of the domain-flow team direction)
Why: Richard/MBJ's GoDaddy setup showed the real business problem: manual DNS can work, but it does not scale if Found is promoted nationally. The team direction is to pursue a GoDaddy-first Domain Connect-style approval flow because it matches the needed owner experience: enter domain, sign into registrar, approve Found's DNS changes, then Found verifies both root and `www`. The previously reverted GoDaddy Personal Access Token path remains rejected for customer-facing use because it asks small business owners to create developer credentials. Namecheap direct API and Cloudflare are not primary customer paths for the same practical reason: they are either too technical for the owner or unfamiliar to the audience. Guardrails: no registrar passwords, no broad DNS credentials, no stored owner registrar API keys unless Shawn and the full team explicitly reopen that decision. Manual DNS remains the production fallback until a no-password approval flow is proven end to end.

**[2026-07-30] - False "Live" domain status fixed - check DNS config, not just ownership.**
Approved by: Shawn (team meeting convened by Shawn, Steve leading, full roster)
Why: Found's "Live" status only ever checked one Vercel signal (domain-ownership claim), never the separate signal for whether DNS is actually configured (Vercel's `/config` endpoint's `misconfigured` field). An untouched domain reads ownership-verified almost immediately, so Found said "Live" for domains not pointing at Found at all - reproduced by Shawn testing exactly as a real owner would, with a real domain (`mambostudio.app`) still on its registrar's parking page. Team review confirmed the actual site-serving path was never affected (Vercel's DNS is the real traffic gate, upstream of Found's code) - this was a dashboard trust bug, not a security or routing bug. Fix requires both signals to agree before reporting "live," fails closed on any error, and adds a grace window before telling an owner their records look wrong (to avoid false alarms during normal DNS propagation). Also logged honestly: the same-day "verified live" test earlier used `example.com`, which structurally cannot expose this gap - see `SESSION_HANDOFF.md` for the corrected record. No real customer was ever shown the false status - every company in the database is currently a test account.

**[2026-07-30] - GoDaddy scoped-token auto-setup REVERTED; manual DNS flow fixed instead; no new registrar automation for now.**
Approved by: Shawn (team meeting convened by Shawn, Steve leading, full roster: Jony, Angela, Craig, Priya, Marcus, Chris, Phil)
Why: The scoped-token auto-setup (see the entry below) shipped, then Shawn immediately rejected it - "My business owners are not developers." No amount of copy polish rescues asking a non-technical small business owner to generate a Developer Portal API token. Reverted in full. Shawn also corrected Claude's process: the prior research/build cycle had been run as an informal, Claude-initiated "team discussion" that excluded Angela (customer journey) and Jony (UX) - the two perspectives that would have caught this immediately - and Claude acted on its own synthesis as if it were settled team consensus. New standing rule: Claude never calls a team meeting; only Shawn does. Shawn then convened a real meeting himself. Team's honest conclusion, Steve's recommendation: hold all new registrar automation (Domain Connect's GoDaddy approval has no promised timeline; Entri Connect costs $3,000-9,000+/year against Found's small current real customer count - neither justified today, and there's no data yet showing domain connection is actually where owners get stuck). Instead, fix the real friction in the already-shipped manual flow: plain-English explanation, a replace-not-duplicate warning, direct registrar DNS-settings links, and an explicit confirmation step. Shawn also confirmed nameserver delegation (switching a domain's authoritative DNS entirely to Found/Vercel) is explicitly out of scope for this fix - simpler-looking on the surface but risks silently breaking any existing business email on the domain, a materially different and riskier architecture than the record-based approach kept here.

**[2026-07-30] - Recommend GoDaddy and Namecheap only, not a 3rd registrar; Squarespace explicitly excluded.**
Approved by: Shawn + Phil Schiller + Marcus Webb
Why: Shawn caught a real conflict in the team's first pass - Squarespace was proposed as a "familiar" 3rd option, but Squarespace is a direct website-builder competitor to Found, not a neutral registrar recommendation. Corrected direction: GoDaddy and Namecheap are the two registrars the team is confident recommending (dominant/familiar with small business owners, not competitors). No 3rd was added just to round out the list - Porkbun was considered (most legitimate non-competitor technical option from the research) but isn't a recognizable "buy a domain here" name for a first-time owner. Copy shipped in `DomainConnector.tsx`'s empty state.

---

**[2026-07-30] - Domain registrar auto-setup: pursue Domain Connect for GoDaddy as the target, scoped API token as the documented fallback - not direct credential storage.**
Approved by: Shawn + Craig Federighi + Priya Nair
Why: Team ruled out asking business owners to hand Found their registrar password/full-account API key outright - Priya's read: an owner's registrar account can transfer or delete their domain, too much blast radius for a UX convenience. Research found two real paths for GoDaddy specifically (chosen as the first target - dominant registrar among Found's likely owners): (1) Domain Connect protocol - GoDaddy co-created this standard and already has a template in the official repo, but becoming a registered service provider requires submitting a template PR and directly contacting GoDaddy's Domain Connect team, an external approval process with unknown timeline; (2) GoDaddy's Developer Platform now issues Personal Access Tokens scoped narrowly to DNS-only writes (opened up April 2026, previously required 50+ domains/reseller status) - available today, no external approval needed, and a real credential-scope improvement over anything previously possible. Decision: pursue Domain Connect registration as the long-term no-credential-storage path, but don't block a v1 build on its approval timeline - the scoped-PAT path is the documented fallback. Actual code for the scoped-PAT flow (credential handling) still needs explicit sign-off before being built, separate from this research decision. See `SESSION_HANDOFF.md` for full research detail.

**[2026-07-30] - Follow-up, same day: built the scoped-PAT path. Shawn approved after confirming the approval bottleneck only applies to Domain Connect, not the direct API.**
Approved by: Shawn
Why: Shawn pushed back on the research above ("how long would this take to get approved") - correctly caught that Claude had conflated Domain Connect's approval requirement with the whole feature being blocked. Re-verified directly against Domain Connect's own docs (approval genuinely required, no self-serve path, confirmed not just assumed) versus GoDaddy's direct v3 API (zero approval needed - any GoDaddy account holder can generate a `domains.dns:update`-scoped Personal Access Token today). With that distinction clear, Shawn approved building the direct-API path immediately ("go ahead approved. build it fast"). Shipped same session: `connectDomainViaGoDaddy()` never persists the token (used once, discarded), resolving Priya's standing credential-custody concern by simply not storing anything. See `SESSION_HANDOFF.md` [2026-07-30] for the technical build detail.
...
**[2026-07-08] - Comp activation before the card prompt: build both link-based and in-flow options, not one.**
Approved by: Shawn + Steve Jobs + Craig Federighi
Why: The first version of this decision (comp *after* onboarding, from `/admin/businesses`) still let the business see a real card-entry screen before Shawn could intervene - a bad look for a live demo. Shawn asked for both options the team laid out rather than picking one, since they're complementary: a comp link (`?comp=<admin key>` on the onboarding URL) that skips the payment screen entirely when Shawn remembers to use it, and an in-flow "Activate as comp" button that appears on the real activation screen whenever Shawn's admin session is present in that browser, regardless of how onboarding started. Both funnel into the same underlying `is_comp` + `subscription_status: active` state and the same server-side `ADMIN_KEY` validation - the client never decides on its own whether comp applies. See `SESSION_HANDOFF.md` for exact usage.
