...
2. **CI build check added** (`.github/workflows/build.yml`) - runs the production build automatically on every push/PR instead of relying purely on manual QA. **Not fully verified** - no GitHub Actions secrets are configured yet and `gh` CLI wasn't available in this environment to set them. Shawn needs to add these under repo Settings > Secrets and variables > Actions, values from `.env.local`: `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`, `NEXT_PUBLIC_ROOT_DOMAIN`, `PEXELS_API_KEY`, `ADMIN_KEY`, `COMP_LINK_SECRET`, `SUPABASE_SERVICE_ROLE_KEY`, `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET_CONNECT`, `SUPABASE_ACCESS_TOKEN`, `SUPABASE_PROJECT_REF`, `CRON_SECRET`, `GOOGLE_PLACES_API_KEY`, `NEXT_PUBLIC_GOOGLE_PLACES_API_KEY`, `STRIPE_PRICE_ID_FOUND_FOUNDING`, `STRIPE_PRICE_ID_FOUND_PRO_FOUNDING`, `STRIPE_PRICE_ID_FOUND_BUSINESS_FOUNDING`. Until those are added, the workflow will run and fail visibly on missing env vars - expected, not a silent problem.
